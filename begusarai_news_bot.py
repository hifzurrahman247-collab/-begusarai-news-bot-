...
def fetch_news_from_web():
    ...
    try:
        response = requests.get("https://www.livehindustan.com/bihar/begusarai", headers=headers, timeout=15)
        if response.status_code == 200:
            ...
            for article in articles[:15]:
                ...
                if text and len(text) > 15 and len(text) < 200:
                    ...
                    news_items.append({'title': text, 'link': href, 'source': 'Live Hindustan', 'time': datetime.now().strftime('%H:%M')})
    except Exception as e:
        logger.error(f"Error fetching Hindustan: {e}")
    
    try:
        response = requests.get("https://www.jagran.com/bihar/begusarai-news-hindi.html", headers=headers, timeout=15)
        if response.status_code == 200:
            soup = BeautifulSoup(response.content, 'html.parser')
            articles = soup.find_all(['a', 'h2', 'h3'], href=True)
            for article in articles[:10]:
                ...
                if text and len(text) > 15 and len(text) < 200:
                    ...
                    news_items.append({'title': text, 'link': href, 'source': 'Dainik Jagran', 'time': datetime.now().strftime('%H:%M')})
    except Exception as e:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    welcome_text = """🙏 *Namaste! Main aapka Begusarai News Bot hoon!* 🇮🇳
...
async def news_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await processing_msg.edit_text(message, parse_mode='Markdown', reply_markup=reply_markup, disable_web_page_preview=True)
...
*News websites par jaane ke liye neeche buttons dabayein:*"""
    keyboard = [
        [InlineKeyboardButton("🏛️ Politics", url='https://www.livehindustan.com/bihar/begusarai'), InlineKeyboardButton("🚔 Crime", url='https://www.jagran.com/bihar/begusarai-news-hindi.html')],
        [InlineKeyboardButton("📚 Education", url='https://www.prabhatkhabar.com/state/bihar/begusarai'), InlineKeyboardButton("🏥 Health", url='https://www.livehindustan.com/bihar/begusarai')],
        [InlineKeyboardButton("🌾 Agriculture", url='https://www.jagran.com/bihar/begusarai-news-hindi.html'), InlineKeyboardButton("🏗️ Development", url='https://www.prabhatkhabar.com/state/bihar/begusarai')],
...
async def about_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    about_text = """ℹ️ *ABOUT BEGUSARAI NEWS BOT* ℹ️
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'latest_news':
        ...
        for i, item in enumerate(news_items[:8], 1):
            ...
            message += f"   📎 [Read More]({item['link']}) | 🏢 {item['source']}\n\n"
        message += "━" * 20 + "\n"
        keyboard = [[InlineKeyboardButton("🔄 Refresh", callback_data='latest_news')], [InlineKeyboardButton("🔔 Subscribe", callback_data='subscribe')]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        try:
            await query.edit_message_text(message, parse_mode='Markdown', reply_markup=reply_markup, disable_web_page_preview=True)
        except:
            await context.bot.send_message(chat_id=update.effective_chat.id, text=message, parse_mode='Markdown', reply_markup=reply_markup, disable_web_page_preview=True)
        ...
        if user_id in subscribed_users:
            ...
            subscribed_users.add(user_id)
            await query.edit_message_text("🎉 *Subscription Successful!*\n\n✅ Aap ab har 2 ghante mein latest Begusarai news paayenge!\n\n🔕 Unsubscribe karne ke liye /unsubscribe type karein.", parse_mode='Markdown')
    elif query.data == 'categories':
        categories_text = "📊 *NEWS CATEGORIES* 📊\n\nNews websites par jaane ke liye neeche buttons dabayein:"
        keyboard = [
        ...
        try:
            ...
            await context.bot.send_message(chat_id=update.effective_chat.id, text=categories_text, parse_mode='Markdown', reply_markup=reply_markup)
    elif query.data == 'websites':
        websites_text = "🌐 *BEGUSARAI NEWS WEBSITES* 🌐\n\n*Official News Portals:*"
        keyboard = [
        ...
        try:
            ...
            await context.bot.send_message(chat_id=update.effective_chat.id, text=websites_text, parse_mode='Markdown', reply_markup=reply_markup)
